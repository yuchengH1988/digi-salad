<script setup>
import awardLogo from '~/assets/images/awards.webp'
</script>
  ...
  <section class="relative z-3 mx-auto mb-[-80px] mt-[-20px] max-w-[1346px] px-6 py-12 text-ink lg:mb-[-216px] lg:mt-[-216px] lg:px-[69px] lg:py-[65px]">
    ...
    <div class="relative z-[2] grid min-h-[302px] grid-cols-1 items-center gap-[37px] lg:grid-cols-[416px_1fr]">
      ...
      <div class="grid grid-cols-2 gap-x-9 gap-y-[19px] max-sm:grid-cols-1" data-fade="up" data-fade-stagger="0.1">
        <img v-for="index in 4" :key="index" data-fade-item :src="awardLogo" :alt="index === 1 ? 'Asia Smart App Awards' : ''" width="4096" height="1561" class="h-[108px] w-full object-contain">
      </div>
    </div>
  </section>
</template>
